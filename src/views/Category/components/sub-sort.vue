<template>
  <div class='sub-sort'>
    ...
    <div class="check">
      <XtxCheckbox @change="chengeCheck" v-model="sortParams.inventory">仅显示有货商品</XtxCheckbox>
      <XtxCheckbox @change="chengeCheck" v-model="sortParams.onlyDiscount">仅显示特惠商品</XtxCheckbox>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const chengeCheck = (sortField) => {
      emit('sort-change', sortField)
    }

    return { sortParams, changeSort, chengeCheck }
  }
}
</script>
